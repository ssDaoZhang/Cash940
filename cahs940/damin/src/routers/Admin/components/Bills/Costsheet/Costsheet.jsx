...
class Costsheet extends Component{
    ...
    getInAmount = (event, index) => {
        ...
        if(numStr.match(/[^\d'.']/g)){
            ...
            this.tableLineNum[index].rawAmount = Math.round(Number(numStr)/ Number(this.selectedRate)) + '';
            this.setState({
                StableLineNum : this.tableLineNum.map(ele => ele)
            });
        }
    ...
    submitMess = (e) => {
        ...
        var sendData = {
            ...
            rate: selectedRate,//汇率
            // dep部门
            payAccount: payAcc.account,//付款账户
            realAmount:payAmount,//实付金额
        ...
        tableLineNum.forEach((ele, index) => {
            sendData.oAmounts.push(ele.inAmount);
            sendData.dAmounts.push(ele.rawAmount);
    ...
    render(){
        ...
        return(
            <div className='costsheet itemwarp'>
                <div className='itemtitle'>制作费用单</div>
                <div className='costsheet__top'>
                    <div className='costsheet__top__sdf'>
                        <div className='costsheet__top__tirps'>
                            <span>制单人:</span>
                            <div>{billMaker}</div>
                        </div>
                        <div className='costsheet__top__calendar'>
                            <span className='costsheettirps'>录单日期:</span>
                            <div className='costsheetdiv'>
                    ...
                    <div className='costsheet__top__item'>
                        <div className='costsheet__top__item__ite'>
                            <span>经手人:</span>
                        ...
                        <div className='costsheet__top__item__ite'>
                            <span>费用币种:</span>
                            <div className='costsheet__top__item__ite__warp'>
                                { dropCurry != '' &&  <DropList 
                                    data={dropCurry}
                                    iconHeight={'1.75em'}
                                    opWarpPosTop={'1.75em'}
                ...
                <div className='costsheet__center'>
                    <table>
                        <thead>
                            <tr>
                                <th>收款账户</th>
                                <th>{seletedCurry.mark ? '详细费用金额(' + seletedCurry.mark + ')' : ''}</th>
                        ...
                        <tbody>{
                            StableLineNum.length != 0 && StableLineNum.map((ele, index) => {
                                return(
                                    <tr key={index}>
                                        <td onClick={e => {this.setInAccFlag(index)}}>
                    ...
                    <div    className='costsheet__center__btn'
                            onClick={this.addLine}
                            >增加一行
                    </div>
                    <div    className='costsheet__center__submit'
