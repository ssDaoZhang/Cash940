...
var FileLoader = function(file, events, getFile, getCall, handDataArr){
...
FileLoader.prototype = {
    ...
    handExcel: function(data){
        ...
        // console.log('obj111', _this);
        // console.log('工作簿流1：', _this.fileflow);
        var carCodeTag = getCarStr({
            data : worksheet,
            searchRang : "A1:E3"
        });
        if(carCodeTag){
            _this.handAjax(carCodeTag);
        }else{
            console.log('木有卡号啊！！！：', carCodeTag);
            _this.fileflow['!ref'] = _this.filelen;
            _this.getFile({file : _this.fileflow, flag : true});
            console.log('木有卡号啊！！！：', carCodeTag);
        }
            
    },
    // selectHandDataFun: (index, carCode) =>{
    //     var rang = this.fileflow['!ref'].replace('A1', this.handDataArr[index].bankT);
    //     // console.log('rang:',rang);
    //     this.fileflow['!ref'] = rang; 

    //     this.handDataArr[index].handHand(
    //         {
    //              data  : JSON.parse(  JSON.stringify(XLSX.utils.sheet_to_json(worksheet))),
    //              carCode : carCode,
